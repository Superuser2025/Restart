...
string BuildJSON(string symbol, ENUM_TIMEFRAMES timeframe)
{
    string json = "{\"type\":\"market_data\",\"timestamp\":" + IntegerToString(TimeCurrent()) +
                  ...
                  ",\"spread\":" + IntegerToString((int)SymbolInfoInteger(symbol, SYMBOL_SPREAD)) + ",\"ohlcv\":[";

    double o[], h[], l[], c[];
    long vol[];  // FIXED: Changed from double[] to long[]

    ArraySetAsSeries(o, true);
    ArraySetAsSeries(h, true);
    ArraySetAsSeries(l, true);
    ArraySetAsSeries(c, true);
    ArraySetAsSeries(vol, true);  // FIXED: This must be long[] for CopyTickVolume

    int bars = 50;
    if(CopyOpen(symbol, timeframe, 0, bars, o) > 0 &&
       CopyHigh(symbol, timeframe, 0, bars, h) > 0 &&
       CopyLow(symbol, timeframe, 0, bars, l) > 0 &&
       CopyClose(symbol, timeframe, 0, bars, c) > 0 &&
       CopyTickVolume(symbol, timeframe, 0, bars, vol) > 0)  // FIXED: vol is now long[]
    {
        for(int i = 0; i < bars; i++)
        {
            if(i > 0) json += ",";
            json += "{\"o\":" + DoubleToString(o[i], 5) +
                    ",\"h\":" + DoubleToString(h[i], 5) +
                    ",\"l\":" + DoubleToString(l[i], 5) +
                    ",\"c\":" + DoubleToString(c[i], 5) +
                    ",\"v\":" + IntegerToString(vol[i]) + "}";  // FIXED: vol[i] is now long
        }
    }
    json += "]}";
    return json;
